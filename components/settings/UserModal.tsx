...
const UserModal: React.FC<UserModalProps> = ({ onClose, onSave, user }) => {
    ...
    const handlePinChange = (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        // Allow only 4 digits for the PIN
        if (/^\d{0,4}$/.test(value)) {
            setPin(value);
        }
    };
                ...
                <form onSubmit={handleSubmit} className="space-y-4">
                     ...
                     <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div>
                         ...
                         <div>
                            <label htmlFor="pin" className="block text-sm font-medium text-slate-700">4-Digit PIN Code (Optional)</label>
                            <input type="text" name="pin" id="pin" value={pin} onChange={handlePinChange} pattern="\d{4}" maxLength={4} className="mt-1 block w-full px-3 py-2 bg-white border border-slate-300 rounded-md shadow-sm" placeholder="e.g. 1234"/>
                        </div>
                    </div>
                     <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                         <div>
                            <label htmlFor="password"className="block text-sm font-medium text-slate-700">Password</label>
                            <input id="password" type="password" required={!isEditMode} value={password} onChange={(e) => setPassword(e.target.value)} className="mt-1 block w-full px-3 py-2 border border-slate-300 rounded-md shadow-sm focus:outline-none focus:ring-emerald-500 focus:border-emerald-500 sm:text-sm" placeholder={isEditMode ? "Leave blank to keep same" : ""}/>
                        </div>
                        <div>
                            <label htmlFor="confirmPassword"className="block text-sm font-medium text-slate-700">Confirm Password</label>
                            <input id="confirmPassword" type="password" required={!isEditMode || password.length > 0} value={confirmPassword} onChange={(e) => setConfirmPassword(e.target.value)} className="mt-1 block w-full px-3 py-2 border border-slate-300 rounded-md shadow-sm focus:outline-none focus:ring-emerald-500 focus:border-emerald-500 sm:text-sm"/>
                        </div>
                    </div>

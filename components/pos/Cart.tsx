...
const Cart = ({ 
    ...
    const { subtotal, discountAmount, taxableAmount, tax, total } = useMemo(
        () => calculateCartTotals(cartItems, discount, settings.tax.vatRate / 100, workOrderDeposit),
        [cartItems, discount, settings.tax.vatRate, workOrderDeposit]
    );
                ...
                <div className="flex justify-between text-sm">
                 ...
                 {workOrderDeposit > 0 && (
                    <div className="flex justify-between text-sm text-accent dark:text-dark-accent">
                        ...
                        <span>- Ksh {workOrderDeposit.toFixed(2)}</span>
                    </div>
                )}
                <div className="flex justify-between text-lg">
                 ...
                 <div className="flex gap-2 mt-4">
                     <motion.button 
                        ...
                        {...{ whileTap: { scale: 0.98 } }}
                        className="flex-1 bg-slate-600 text-white font-bold py-3 rounded-lg hover:bg-slate-700 transition-colors disabled:bg-slate-400 dark:disabled:bg-slate-600 flex items-center justify-center text-lg shadow-md"
                     >
                        Hold
                     </motion.button>
                     <motion.button 
                        ...
                        disabled={cartItems.length === 0 || !activeShift}
                        {...{ whileTap: { scale: 0.98 } }}
                        className="flex-[2] bg-primary dark:bg-dark-primary text-white font-bold py-3 rounded-lg hover:bg-blue-800 dark:hover:bg-blue-500 transition-colors disabled:bg-slate-400 dark:disabled:bg-slate-600 flex items-center justify-center text-lg shadow-lg"
                     >
                         Charge

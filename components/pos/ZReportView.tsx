...
import { Shift, Sale, Settings, Payout } from '../../types';
...
const ZReportView: React.FC<ZReportViewProps> = ({ shift, sales: allSales, payouts: allPayouts, onClose, settings, isHistoricalView = false }) => {
    const reportRef = useRef<HTMLDivElement>(null);
    const [isDownloading, setIsDownloading] = useState(false);
    
    const shiftSales = allSales.filter(s => shift.salesIds.includes(s.id));
    const shiftPayouts = allPayouts.filter(p => shift.payoutIds?.includes(p.id));
            ...
            <div className="max-w-4xl mx-auto">
                <div id="shift-report-container" ref={reportRef} className="bg-card dark:bg-dark-card p-6 md:p-8 rounded-xl shadow-lg space-y-6">
                    <div className="text-center border-b dark:border-dark-border pb-4">
                        ...
                        <div className="grid grid-cols-2 gap-4 text-xs mt-2 text-foreground/70 dark:text-dark-foreground/70">
                            <div className="text-left">
                                <p className="font-bold">Fiscal Day Started:</p>
                                <p>{new Date(shift.startTime).toLocaleString('en-GB', { timeZone: 'Africa/Nairobi' })}</p>
                            </div>
                            <div className="text-right">
                                <p className="font-bold">Fiscal Day Ended:</p>
                                <p>{shift.endTime ? new Date(shift.endTime).toLocaleString('en-GB', { timeZone: 'Africa/Nairobi' }) : 'Active'}</p>
                            </div>
                        </div>
                    </div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                        ...
                        <StatCard title="Total Payouts" value={formatCurrency(shift.totalPayouts || 0)} />
                    ...
                    <div>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                            ...
                            <div className="space-y-1 bg-background dark:bg-dark-background/50 p-4 rounded-lg text-sm">
                                ...
                                <div className="flex justify-between text-danger"><span className="dark:text-danger">- Payouts</span><span className="font-mono">{formatCurrency(shift.totalPayouts || 0)}</span></div>
                     ...
                     {shiftPayouts.length > 0 && (
                        <div>
                            <h3 className="text-lg font-semibold text-foreground dark:text-dark-foreground mb-2">Payouts During Shift</h3>
                            <div className="overflow-x-auto rounded-lg border dark:border-dark-border max-h-40">
                                <table className="w-full text-sm text-left text-foreground/80 dark:text-dark-foreground/80">
                                    <thead className="text-xs text-foreground/90 dark:text-dark-foreground/90 uppercase bg-background dark:bg-dark-card/60 sticky top-0">
                                        <tr>
                                            ...
                                            <th scope="col" className="px-4 py-2">Payee</th>
                                            <th scope="col" className="px-4 py-2 text-right">Amount</th>
                                        </tr>
                                    </thead>
                                    <tbody>
                                        {shiftPayouts.map(payout => (
                                            <tr key={payout.id} className="bg-card dark:bg-dark-card border-b dark:border-dark-border hover:bg-background/50 dark:hover:bg-dark-background/50">
                                                <td className="px-4 py-2 font-medium text-foreground dark:text-dark-foreground">{payout.reason}</td>
                                                <td className="px-4 py-2">{payout.payee || 'N/A'}</td>
                                                <td className="px-4 py-2 text-right font-mono text-danger">{formatCurrency(payout.amount)}</td>
                                            </tr>
                                        ))}
                                    </tbody>
                                </table>
